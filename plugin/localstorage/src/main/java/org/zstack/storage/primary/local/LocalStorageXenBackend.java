...
import org.zstack.storage.primary.local.LocalStorageXenMigrateVmFlow.CopyBitsFromRemoteCmd;
 ...
 */
public class LocalStorageXenBackend extends LocalStorageHypervisorBackend {
    private final static CLogger logger = Utils.getLogger(LocalStorageXenBackend.class);
    ...
    public LocalStorageXenBackend(PrimaryStorageVO self) {
        super(self);
    }
    ...
    void syncPhysicalCapacityInCluster(List<ClusterInventory> clusters, final ReturnValueCompletion<PhysicalCapacityUsage> completion) {
        ...
        List<XenHostAsyncHttpCallMsg> msgs = CollectionUtils.transformToList(hostUuids, new Function<XenHostAsyncHttpCallMsg, String>() {
            @Override
            public XenHostAsyncHttpCallMsg call(String arg) {
        ...
        bus.send(msg, new CloudBusCallBack(completion) {
            ...
            public void run(MessageReply reply) {
                ...
                if (rsp.getTotalCapacity() != null && rsp.getAvailableCapacity() != null) {
                    new LocalStorageCapacityUpdater().updatePhysicalCapacityByXenAgentResponse(self.getUuid(), hostUuid, rsp);
                }
        ...
        httpCall(CREATE_EMPTY_VOLUME_PATH, hostUuid, cmd, CreateEmptyVolumeRsp.class, new ReturnValueCompletion<CreateEmptyVolumeRsp>(completion) {
            ...
            public void fail(ErrorCode errorCode) {
                completion.fail(errf.instantiateErrorCode(SysErrors.OPERATION_ERROR,
                        String.format("unable to create an empty volume[uuid:%s, name:%s] on the Xen host[uuid:%s]",
                    ...
                    fchain.then(new ShareFlow() {
                        ...
                        public void setup() {
                            ...
                            flow(new NoRollbackFlow() {
                                ...
                                @Override
                                public void run(final FlowTrigger trigger, Map data) {
                                    LocalStorageBackupStorageMediator m = localStorageFactory.getBackupStorageMediator(XenConstant.KVM_HYPERVISOR_TYPE, backupStorage.getType());
    ...
    private void createRootVolume(final InstantiateRootVolumeFromTemplateOnPrimaryStorageMsg msg, final ReturnValueCompletion<InstantiateVolumeOnPrimaryStorageReply> completion) {
        ...
        chain.setName(String.format("Xen-localstorage-create-root-volume-from-image-%s", image.getUuid()));
    ...
    void handle(final DownloadDataVolumeToPrimaryStorageMsg msg, final ReturnValueCompletion<DownloadDataVolumeToPrimaryStorageReply> completion) {
        ...
        LocalStorageBackupStorageMediator m = localStorageFactory.getBackupStorageMediator(XenConstant.KVM_HYPERVISOR_TYPE, bsvo.getType());
    ...
    void handle(BackupVolumeSnapshotFromPrimaryStorageToBackupStorageMsg msg, String hostUuid, final ReturnValueCompletion<BackupVolumeSnapshotFromPrimaryStorageToBackupStorageReply> completion) {
        VolumeSnapshotInventory sp = msg.getSnapshot();
        LocalStorageBackupStorageMediator m = localStorageFactory.getBackupStorageMediator(XenConstant.KVM_HYPERVISOR_TYPE, msg.getBackupStorage().getType());
                    ...
                    private void migrate(final FlowTrigger trigger) {
                        ...
                        thdf.chainSubmit(new ChainTask(trigger) {
                            ...
                            public void run(final SyncTaskChain chain) {
                                ...
                                httpCall(LocalStorageXenMigrateVmFlow.COPY_TO_REMOTE_BITS_PATH, struct.getSrcHostUuid(), cmd, false,
