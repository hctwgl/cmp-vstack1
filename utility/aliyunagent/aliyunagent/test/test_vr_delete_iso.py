...
from aliyunagent.plugins import virtualrouter_plugin
...
class Test(unittest.TestCase):
    ...
    def testName(self):
        url = aliyunagent._build_url_for_test([virtualrouter_plugin.VirtualRouterPlugin.VR_KVM_CREATE_BOOTSTRAP_ISO_PATH])
        ...
        cmd.isoPath = '/tmp/vr.iso'
        url = aliyunagent._build_url_for_test([virtualrouter_plugin.VirtualRouterPlugin.VR_KVM_DELETE_BOOTSTRAP_ISO_PATH])
        rsp = http.json_dump_post(url, cmd, headers={http.TASK_UUID:uuidhelper.uuid(), http.CALLBACK_URI:self.CALLBACK_URL})
        time.sleep(2)
        self.service.stop()
