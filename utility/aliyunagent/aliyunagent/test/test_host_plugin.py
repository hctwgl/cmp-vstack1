...
from aliyunagent.plugins import host_plugin
...
class ConnectCmd(aliyunagent.AgentCommand):
    def __init__(self):
        self.hostUuid = uuidhelper.uuid()
        
class HostFactCmd(aliyunagent.AgentCommand): pass
...
class TestHostPlugin(unittest.TestCase):
    ...
    def test_connect(self):
        url = aliyunagent._build_url_for_test([host_plugin.CONNECT_PATH])
    ...
    def test_hostfact(self):
        url = aliyunagent._build_url_for_test([host_plugin.HOSTFACT_PATH])
        cmd = HostFactCmd()
