...
class ConnectResponse(ecsagent.AgentResponse):
    def __init__(self):
        super(ConnectResponse, self).__init__()

class HostCapacityResponse(ecsagent.AgentResponse):
    def __init__(self):
...
class HostFactResponse(ecsagent.AgentResponse):
    def __init__(self):
...
class SetupMountablePrimaryStorageHeartbeatCmd(ecsagent.AgentCommand):
    def __init__(self):
        super(SetupMountablePrimaryStorageHeartbeatCmd, self).__init__()
        self.heartbeatFilePaths = None
        self.heartbeatInterval = None

class SetupMountablePrimaryStorageHeartbeatResponse(ecsagent.AgentResponse):
    def __init__(self):
        super(SetupMountablePrimaryStorageHeartbeatResponse, self).__init__()

class PingResponse(ecsagent.AgentResponse):
    def __init__(self):
...
class HostPlugin(ecsagent.AliyunAgent):
    '''
    classdocs
    '''
    ...
    def _get_qemu_version(self):
        ret = shell.call('%s -version' % ecsagent.get_qemu_path())
        words = ret.split()
        for w in words:
            if w == 'version':
                return words[words.index(w)+1].strip()

        raise ecsagent.KvmError('cannot get qemu version[%s]' % ret)

    @ecsagent.replyerror
    def connect(self, req):
        cmd = jsonobject.loads(req[http.REQUEST_BODY])
        self.host_uuid = cmd.hostUuid
        self.config[ecsagent.HOST_UUID] = self.host_uuid
        self.config[ecsagent.SEND_COMMAND_URL] = cmd.sendCommandUrl
