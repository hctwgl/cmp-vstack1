...
# create log
logger_dir = "/var/log/Xen/"
create_log(logger_dir)
banner("Starting to deploy xen agent")
...
virtenv_path = "%s/virtualenv/xen" % zstack_root
workplace = "%s/xen" % zstack_root
...
copy_arg = CopyArg()
copy_arg.src = "files/xen/zstack-xenagent"
...
if chroot_env == 'false':
    ...
    if distro == "RedHat" or distro == "CentOS":
        command = "service zstack-xenagent stop && service zstack-xenagent start && chkconfig zstack-xenagent on"
    elif distro == "Debian" or distro == "Ubuntu":
    ...
    host_post_info.post_label_param = "zstack-xenagent"
    run_remote_command(command, host_post_info)
