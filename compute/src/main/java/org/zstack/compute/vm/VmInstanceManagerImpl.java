...
public class VmInstanceManagerImpl extends AbstractService
		implements VmInstanceManager, ReportQuotaExtensionPoint, ManagementNodeReadyExtensionPoint,
		L3NetworkDeleteExtensionPoint, ResourceOwnerAfterChangeExtensionPoint, GlobalApiMessageInterceptor,
		VmInstanceDestroyExtensionPoint, RecoverVmExtensionPoint, VmBeforeExpungeExtensionPoint {
	private static final CLogger logger = Utils.getLogger(VmInstanceManagerImpl.class);
	private Map<String, VmInstanceFactory> vmInstanceFactories = Collections
			.synchronizedMap(new HashMap<String, VmInstanceFactory>());
	private Map<String, VmPubInstanceFactory> vmPubInstanceFactories = Collections
			.synchronizedMap(new HashMap<String, VmPubInstanceFactory>());
	...
	private FlowChainBuilder createPubVmFlowBuilder;
	...
	void passThrough(VmInstanceMessage msg) {
		...
		if ((msg instanceof StartNewCreatedPubVmInstanceMsg) || (msg instanceof UpdatePubVmInstanceMsg)) {
			...
			if (vo == null) {
				String err = String.format("Cannot find VmInstance[uuid:%s], it may have been deleted",
	...
	private void getInterdependentL3NetworksByImageUuid(APIGetInterdependentL3NetworksImagesMsg msg) {
		...
		if (bssTypes.isEmpty()) {
			throw new OperationFailureException(errf.stringToInvalidArgumentError(String.format(
					"the image[uuid:%s] is not on any backup storage that has been attached to the zone[uuid:%s]",
	...
	private void handle(APIGetCandidateZonesClustersHostsForCreatingVmMsg msg) {
		...
		if (image.getBackupStorageRefs().size() == 1) {
			...
			if (msg.getZoneUuid() == null) {
				throw new OperationFailureException(errf.stringToInvalidArgumentError(String.format(
						"zoneUuid must be set because the image[name:%s, uuid:%s] is on multiple backup storage",
						image.getName(), image.getUuid())));
			}
		...
		vm.setDefaultL3NetworkUuid(
				msg.getDefaultL3NetworkUuid() == null ? msg.getL3NetworkUuids().get(0) : msg.getDefaultL3NetworkUuid());
			...
			public void run(MessageReply reply) {
				if (!reply.isSuccess()) {
					...
					if (!re.getHosts().isEmpty()) {
						...
						List<String> clusterUuids = re.getHosts().stream().map(HostInventory::getClusterUuid)
								.collect(Collectors.toList());
						areply.setClusters(
								ClusterInventory.valueOf(dbf.listByPrimaryKeys(clusterUuids, ClusterVO.class)));

						List<String> zoneUuids = re.getHosts().stream().map(HostInventory::getZoneUuid)
								.collect(Collectors.toList());
	...
	private void doCreateVmInstance(final CreateVmInstanceMsg msg, final APICreateMessage cmsg,
			ReturnValueCompletion<CreateVmInstanceReply> completion) {
		...
		if (msg.getConsolePassword() != null) {
			VmSystemTags.CONSOLE_PASSWORD.recreateInherentTag(vo.getUuid(),
		...
		if (instanceOfferingUuid != null) {
			tagMgr.copySystemTag(instanceOfferingUuid, InstanceOfferingVO.class.getSimpleName(), vo.getUuid(),
	...
	private void doUpdatePubVmInstance(final APIQueryMessage cmsg,
			ReturnValueCompletion<UpdatePubVmInstanceReply> completion) {
	...
	private void doCreatePublicVmInstance(final CreatePubVMInstanceMsg msg, final APICreateMessage cmsg,
		...
		// if (msg.getConsolePassword() != null) {
		// VmSystemTags.CONSOLE_PASSWORD.recreateInherentTag(vo.getUuid(),
		// map(e(VmSystemTags.CONSOLE_PASSWORD_TOKEN,
		// msg.getConsolePassword())));
		// }
	...
	private void createVmFlowChainBuilder()
		...
		createPubVmFlowBuilder = FlowChainBuilder.newBuilder().setFlowClassNames(createPubVmWorkFlowElements)
				.construct();
		...
		attachVolumeFlowBuilder = FlowChainBuilder.newBuilder().setFlowClassNames(attachVolumeWorkFlowElements)
				.construct();
		...
		class HostNameValidator implements SystemTagCreateMessageValidator, SystemTagValidator {
			private void validateHostname(String tag, String hostname) {
				...
				if (!domainValidator.isValid(hostname)) {
					throw new ApiMessageInterceptionException(errf.stringToInvalidArgumentError(String.format(
							"hostname[%s] specified in system tag[%s] is not a valid domain name", hostname, tag)));
				}
			}
			...
			public void validateSystemTagInCreateMessage(APICreateMessage cmsg) {
				...
				for (String sysTag : msg.getSystemTags()) {
					if (VmSystemTags.HOSTNAME.isMatch(sysTag)) {
						if (++hostnameCount > 1) {
							throw new ApiMessageInterceptionException(errf.stringToInvalidArgumentError(String
									.format("only one hostname system tag is allowed, but %s got", hostnameCount)));
						}
			...
			private void validateStaticIp(String sysTag) {
				...
				if (!dbf.isExist(l3Uuid, L3NetworkVO.class)) {
					throw new ApiMessageInterceptionException(errf.stringToInvalidArgumentError(String.format(
							"L3 network[uuid:%s] not found. Please correct your system tag[%s] of static IP", l3Uuid,
							sysTag)));
				}
				...
				if (!NetworkUtils.isIpv4Address(ip)) {
					throw new ApiMessageInterceptionException(errf.stringToInvalidArgumentError(String.format(
							"%s is not a valid IPv4 address. Please correct your system tag[%s] of static IP", ip,
							sysTag)));
				}
				...
				if (!r.isSuccess()) {
					throw new ApiMessageInterceptionException(
							errf.instantiateErrorCode(SysErrors.INTERNAL, r.getError()));
				}
			...
			private void validateHostNameOnDefaultL3Network(String tag, String hostname, String l3Uuid) {
				...
				if (!vos.isEmpty()) {
					...
					throw new ApiMessageInterceptionException(errf.stringToInvalidArgumentError(String.format(
							"conflict hostname in system tag[%s]; there has been a VM[uuid:%s] having hostname[%s] on L3 network[uuid:%s]",
	...
	protected void putVmToUnknownState(final String hostUuid) {
		...
		List<VmStateChangedOnHostMsg> msgs = CollectionUtils.transformToList(tss,
				new Function<VmStateChangedOnHostMsg, String>() {
					@Override
	...
	@Override
	public FlowChain getCreatePubVmWorkFlowChain() {
		return createPubVmFlowBuilder.build();
	}
		...
		QuotaOperator checker = new QuotaOperator() {
			...
			private VmQuota getUsedVmCpuMemory(String accountUUid) {
				...
				String sql = "select count(vm), sum(vm.cpuNum), sum(vm.memorySize)"
						+ " from VmInstanceVO vm, AccountResourceRefVO ref where" + " vm.uuid = ref.resourceUuid"
				...
				q.setParameter("states",
						list(VmInstanceState.Stopped, VmInstanceState.Destroying, VmInstanceState.Destroyed));
			...
			private void check(APIStartVmInstanceMsg msg, Map<String, Quota.QuotaPair> pairs) {
				...
				if (vmQuota.vmNum + 1 > vmNum) {
					throw new ApiMessageInterceptionException(errf.instantiateErrorCode(IdentityErrors.QUOTA_EXCEEDING,
							String.format(
									"quota exceeding. Current account is [uuid: %s]. "
											+ "The resource owner account[uuid: %s] exceeds a quota[name: %s, value: %s]",
									currentAccountUuid, resourceOwnerAccountUuid, VmInstanceConstant.QUOTA_VM_NUM,
									vmNum)));
				}
				...
				if (vmQuota.cpuNum + cpuNumAsked > cpuNum) {
					throw new ApiMessageInterceptionException(errf.instantiateErrorCode(IdentityErrors.QUOTA_EXCEEDING,
							String.format(
									"quota exceeding. Current account is [uuid: %s]. "
											+ "The resource owner account[uuid: %s] exceeds a quota[name: %s, value: %s]",
									currentAccountUuid, resourceOwnerAccountUuid, VmInstanceConstant.QUOTA_CPU_NUM,
									cpuNum)));
				}
				...
				if (vmQuota.memorySize + memoryAsked > memory) {
					throw new ApiMessageInterceptionException(errf.instantiateErrorCode(IdentityErrors.QUOTA_EXCEEDING,
							String.format(
									"quota exceeding. Current account is [uuid: %s]. "
											+ "The resource owner account[uuid: %s] exceeds a quota[name: %s, value: %s]",
									currentAccountUuid, resourceOwnerAccountUuid, VmInstanceConstant.QUOTA_VM_MEMORY,
									memory)));
				}
			}
			...
			private void check(APIChangeResourceOwnerMsg msg, Map<String, Quota.QuotaPair> pairs) {
				...
				if (resourceTargetOwnerAccountUuid.equals(resourceOriginalOwnerAccountUuid)) {
					throw new ApiMessageInterceptionException(errf.instantiateErrorCode(IdentityErrors.QUOTA_INVALID_OP,
							String.format("Invalid ChangerResourceOwner operation."
									+ "Original owner is the same as target owner." + "Current account is [uuid: %s]."
									+ "The resource target owner account[uuid: %s]."
									+ "The resource original owner account[uuid:%s].", currentAccountUuid,
									resourceTargetOwnerAccountUuid, resourceOriginalOwnerAccountUuid)));
				}
				...
				if (accResRefVO.getResourceType().equals(VolumeVO.class.getSimpleName())) {
					...
					if (n + 1 > volNum) {
						throw new ApiMessageInterceptionException(
								errf.instantiateErrorCode(IdentityErrors.QUOTA_EXCEEDING,
										String.format(
												"quota exceeding. Current account is [uuid: %s]. "
														+ "The resource owner account[uuid: %s] exceeds a quota[name: %s, value: %s]",
			...
			@Transactional(readOnly = true)
			private void checkVolumeQuota(String currentAccountUuid, String resourceOwnerAccountUuid, String volumeUuid,
				...
				if (n + 1 > volNum) {
					throw new ApiMessageInterceptionException(errf.instantiateErrorCode(IdentityErrors.QUOTA_EXCEEDING,
							String.format(
									"quota exceeding. Current account is [uuid: %s]. "
											+ "The resource owner account[uuid: %s] exceeds a quota[name: %s, value: %s]",
									currentAccountUuid, resourceOwnerAccountUuid, VolumeConstant.QUOTA_DATA_VOLUME_NUM,
									volNum)));
				}
				...
				if (vsize + requiredVolSize > volSize) {
					throw new ApiMessageInterceptionException(errf.instantiateErrorCode(IdentityErrors.QUOTA_EXCEEDING,
							String.format(
									"quota exceeding. Current account is [uuid: %s]. "
											+ "The resource owner account[uuid: %s] exceeds a quota[name: %s, value: %s]",
									currentAccountUuid, resourceOwnerAccountUuid, VolumeConstant.QUOTA_VOLUME_SIZE,
									volSize)));
				}
			}
		...
		for (int i = 0; i < times; i++) {
			...
			for (Tuple t : lst) {
				...
				if (hostUuid == null) {
					// TODO
					logger.warn(String
							.format("the vm[uuid:%s] is in Unknown state, but its hostUuid is null, we cannot check its"
									+ " real state", vmUuid));
					continue;
				}
					...
					bus.send(msg, new CloudBusCallBack() {
						...
						public void run(MessageReply reply) {
							if (reply.isSuccess()) {
								...
								completion.fail(errf.stringToOperationError(String.format(
										"unable to check the vm[uuid:%s]'s state, %s", uuid, reply.getError())));
							}
						}
					});
				}
	...
	private synchronized void startVmExpungeTask() {
		...
		expungeVmTask = thdf.submitCancelablePeriodicTask(new CancelablePeriodicTask() {
			...
			public synchronized boolean run() {
				...
				final List<ExpungeVmMsg> msgs = CollectionUtils.transformToList(vms,
						new Function<ExpungeVmMsg, Tuple>() {
							@Override
							public ExpungeVmMsg call(Tuple t) {
								String uuid = t.get(0, String.class);
								Timestamp date = t.get(1, Timestamp.class);
								long end = date.getTime()
										+ TimeUnit.SECONDS.toMillis(VmGlobalConfig.VM_EXPUNGE_PERIOD.value(Long.class));
		...
		logger.debug(String.format("vm expunging task starts running, [period: %s seconds, interval: %s seconds]",
				VmGlobalConfig.VM_EXPUNGE_PERIOD.value(Long.class),
				VmGlobalConfig.VM_EXPUNGE_INTERVAL.value(Long.class)));
	}
		...
		if (vq.isExists()) {
			throw new OperationFailureException(
					errf.stringToOperationError(
							String.format(
									"the resource[uuid:%s] is a ROOT volume, you cannot change its owner, instead,"
											+ "change the owner of the VM the root volume belongs to",
									ref.getResourceUuid())));
		}
	}
